...
class ChaparPostRateCalculator implements ShippingRateCalculatorInterface
{

    public function __construct(
        private int $baseTariff = 250000,
        private int $perKgRate = 20000,
        private int $insuranceFee = 5000
    ) {
    }
    ...
    public function calculate(string $orderId, float $weightInKg, string $destinationCity): int
    {
        ...
        echo sprintf(
            "[Chapar Rate] Order #%s to %s (%0.2f kg) calculated tariff: %s IRR\n",
            $orderId,
            $destinationCity,
            $weightInKg,
